...
class source:
    ...
    def results(self, data):
        ...
        try:
            ...
            from modules.kodi_utils import logger
            logger('davio scraper Exception', str(e))
            return sources
        ...
        for item in results:
            try:
                ...
                try: file_name = description.split('\n')[0]
                ...
                try: isize = _size(size)
                except: isize = 0
                quality, info = source_utils.get_file_info(name_info)
                
                append({'name': file_name, 'display_name': origname, 'quality': '1080p', 'size': isize, 'size_label': size,
									'extraInfo': info, 'url_dl': url, 'down_url': url, 'id': url, 'local': False, 'direct': True, 'source': 'Davio',
